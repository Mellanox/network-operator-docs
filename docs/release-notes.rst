...
.. list-table::
   ...
   * - Version
     - Description
   * - 26.1.1
     - | - There is a known limitation in SR-IOV OVSNetwork, opposed of using SriovNetwork or SriovIBNetwork. When using OVSNetwork, setting VF link state to follow its PF link state is not supported
   * - 26.1.0
   ...
   * - 1.1.0
     - | - NicClusterPolicy update is not supported at the moment
       ...
       | - GPUDirect could have performance degradation if it is used with servers which are not optimized. Please see official GPUDirect documentation `here <https://docs.nvidia.com/cuda/gpudirect-rdma/index.html#supported-systems>`__
